...
A **production-ready Streamlit web application** that helps students at the Technological Institute of the Philippines (T.I.P.) find information from their Student Manual using **semantic search** and **in-context classification**.
...
- 🔒 **100% Local**: No API keys, no internet connection required after initial setup
...
4. **Provide Feedback**: Click "Helpful" or "Not Helpful" buttons
   ...
   - Analytics are shown in the sidebar
...
- **Confidence Indicators**: 
  - ✅ Green card = High confidence (>60%)
  - ⚠️ Orange card = Lower confidence (<60%)
...
```
- Searches through all manual chunks using L2 distance
...
- **Response Time**: <2 seconds on modern hardware
...
```python
top_chunks, sim_scores = retrieve_chunks(question, model, chunks, index, chunk_embeds, top_k=5)
```
...
### 3. `feedback_log.csv` (auto-generated)
...
- Use smaller embedding model (e.g., `all-MiniLM-L3-v2`)
- Enable caching (already implemented with `@st.cache_resource`)
...
- **HCI Principles**: Clean UI, feedback loops, transparency (confidence scores)
- **Production Practices**: Caching, error handling, modular code
- **No-LLM Approach**: Uses retrieval and similarity, not generative AI
...
4. Ensure Python 3.10+ and all dependencies installed
